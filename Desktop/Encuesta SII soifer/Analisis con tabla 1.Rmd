...
encuesta_tabla1 <- encuesta %>%
                  mutate(`Hospital` = if_else(str_detect(`Lugar de Trabajo`, regex("hospital", ignore_case = T)), "Si", "No"),
                         `Consultorio privado` = if_else(str_detect(`Lugar de Trabajo`, regex("consultorio", ignore_case = T)), "Si", "No"),
                         `Centro médico privado` = if_else(str_detect(`Lugar de Trabajo`, regex("centro", ignore_case = T)), "Si", "No"))

listVars <- c("Edad", "Región de residencia", "Orientación dicotomica", "Hospital", "Centro médico privado", "Consultorio privado")

catVars <- c("Región de residencia", "Orientación dicotomica", "Hospital", "Centro médico privado", "Consultorio privado")

table1 <- CreateTableOne(vars = listVars, data = encuesta_tabla1, factorVars = catVars, strata = c("Genero"), test = F)

a <- print(table1, quote = F, noSpaces = T)

as.data.frame(a)

tabla_uno <- a %>%
          kbl(caption = "Tabla 1: descripción de los médicos encuestados") %>%
   kable_classic(full_width = F, html_font = "Cambria")
...
## intento modificar lugar de trabajo para tabla 1

```{r}
tabla_trabajo1 <- encuesta  %>%
  mutate()
  mtabulate(strsplit(encuesta$`Lugar de Trabajo`, " y ")) %>%
                         summarise_all(.funs = sum,na.rm=T) 
```
