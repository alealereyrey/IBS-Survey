...
piramide <- ggplot(dfpiramide2, aes(x = Edad_ordinal, y = Especialistas, fill = Genero)) + 
  ...
  scale_fill_manual(values = c("cornflowerblue", "#E76BF3")) +
        labs(y = "Nro de especialistas", x = "Edad", fill = "Género") +
...
## Grafico la fisiopatologia de acuerdo a la especialidad


```{r}

diarrea <- encuesta[,c(14,18:32, 74:79, 81)]
...
grafico_fisiopato_diarrea <- ggplot(fisiopato_diarrea, aes(x= `Fisiopatología más frecuente variante diarrea`, y = Proporcion, fill = `Orientación dicotómica`)) +
        geom_col(na.rm = T, show.legend = T, position = "dodge") + 
        geom_text(position = position_dodge(width = 1), aes (label = `Proporcion`), vjust = 0.5, hjust = -0.1) +
                        scale_x_discrete(na.translate = FALSE) +
                        theme_bw() +
                        coord_flip() +
        labs(title = "Fisiopatología más frecuente del SII-D", subtitle = "p-value = 0.5883", x = "", y = "%", fill = "Orientación") +
        expand_limits(y = c(0, 60))
...
grafico_fisiopato_constipacion <- ggplot(fisiopato_constipacion, aes(x= `Fisiopatología más frecuente variante constipación`, y = Proporcion, fill = `Orientación dicotómica`)) +
                        geom_col(na.rm = T, show.legend = T, position = "dodge") + 
        geom_text(position = position_dodge(width = 1), aes (label = `Proporcion`), vjust = 0.5, hjust = -0.1) +
                        scale_x_discrete(na.translate = FALSE) +
                        theme_bw() +
                        coord_flip() +
        labs(title = "Fisiopatología más frecuente del SII-C", subtitle = "p-value = 0.4221", x = "", y = "%", fill = "Orientación") +
        expand_limits(y = c(0, 50))


grafico_fisiopato_constipacion   

t.test(Proporcion ~`Orientación dicotómica`, data=fisiopato_diarrea )

t.test(Proporcion ~`Orientación dicotómica`, data=fisiopato_constipacion )
```
...
```{r}
tto_diarrea_neurogastro<- encuesta[,c(18:32,81)] %>%
...
tto_diarrea_otros <- encuesta[,c(18:32,81)] %>%
        filter(`Orientación dicotómica` != "Neurogastro") %>%
        pivot_longer(cols = everything()) %>%
          count(name, value) %>%
  pivot_wider(names_from = value, values_from = n, values_fill = 0) %>%
...
tto_diarrea_neurogastro<- tto_diarrea_neurogastro[-(16),]
tto_diarrea_otros <- tto_diarrea_otros[-(16),]
...
# remuevo "en diarrea"
df_final$name <- str_remove(df_final$name, " en diarrea")
...
```{r}
tto_constipacion_neurogastro<- encuesta[,c(33:43,81)] %>%
        filter(`Orientación dicotómica` == "Neurogastro") %>%
        pivot_longer(cols = everything()) %>%
          count(name, value) %>%
  pivot_wider(names_from = value, values_from = n, values_fill = 0) %>%
...
tto_constipacion_otros <- encuesta[,c(33:43,81)] %>%
        filter(`Orientación dicotómica` != "Neurogastro") %>%
        pivot_longer(cols = everything()) %>%
          count(name, value) %>%
  pivot_wider(names_from = value, values_from = n, values_fill = 0) %>%
...
tto_constipacion_otros <- tto_constipacion_otros[-(12),]

# remuevo nombre de la primera columna
colnames(tto_constipacion_neurogastro)[1] <- ""
colnames(tto_constipacion_otros)[1] <- ""
...
df_tto_constipacion <- cbind(tto_constipacion_neurogastro,tto_constipacion_otros) 
...
# remuevo "en diarrea"
df_final_constipacion$name <- str_remove(df_final_constipacion$name, " en diarrea")
