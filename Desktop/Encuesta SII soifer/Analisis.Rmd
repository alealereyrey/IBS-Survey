...
encuesta <- encuesta %>%
                mutate(`Provincia de residencia` = case_when(str_detect(`Provincia de residencia`, regex("caba|C.A.B.A|ciudad|capital|a(i|u)res|bs as", ignore_case = T)) ~"Buenos Aires", 
                       ...
                       `Ciudad donde ejerce`= case_when(str_detect(`Ciudad donde ejerce`, regex("C.A.B.A.|CABA|Buenos|bs as|Capital|Morón|Pilar|Lujan|La plata|Varela|Mejía|Quilmes|Palomar|Isidro|Vicente|cas|Zamora|Campana|argentina", ignore_case = T)) ~ "AMBA",
                                                        str_detect(`Ciudad donde ejerce`, regex("mar del plata", ignore_case = T)) ~ "Mar del plata",
...
distribucion_residencia <- encuesta%>%
                        group_by(Genero)%>%
                        count(`Provincia de residencia`, `Ciudad donde ejerce`, name = "Especialistas") %>%
                        ungroup()
...
grafico_residencia <- ggplot(distribucion_residencia, aes(x = `Provincia de residencia`, y = Especialistas, fill = Genero)) +
                   ...
                   labs(title = "Provincia de residencia") +
                           scale_fill_manual(values = c("cornflowerblue", "#E76BF3"))
...
grafico_ciudad_buenos_aires <- ggplot(ciudad_buenos_aires,
                        ...
                        geom_bar(position = "dodge", stat = "identity") +
                              theme_bw() +
        coord_flip()
        labs(title = "Ciudad de ejercicio de la profesión") 
...
grafico_resto_del_pais<- ggplot(resto_del_pais,
                        ...
                        coord_flip()
                         labs(title = "Ciudad de ejercicio de la profesión") +
                facet_grid(~`Provincia de residencia`)
