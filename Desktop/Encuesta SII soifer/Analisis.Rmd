...
fisiopato_diarrea <- diarrea %>%
         group_by(`Fisiopatología más frecuente variante diarrea`) %>%
                        ...
                        mutate(Proporcion = round((n/sum(n))*100,1)) %>%
                        ungroup()
grafico_fisiopato_diarrea <- ggplot(fisiopato, aes(x= `Fisiopatología más frecuente variante diarrea`, y = Proporcion, fill = Proporcion)) +
...
proporcion <-"Proporcion de pacientes con SII mensual"

encuesta[proporcion] <- lapply(encuesta[proporcion], factor, levels = c("0 a 25%", "25 a 50%", "> de 50%"), ordered = T)

proporcion_sii <- encuesta %>%
        group_by(Edad_ordinal,`Proporcion de pacientes con SII mensual`)%>%
        summarise(n = n()) %>%
        mutate(Proporcion = round((n/sum(n))*100,1))

ggplot(proporcion_sii, aes(fill = `Proporcion de pacientes con SII mensual`, y = Proporcion,  x = Edad_ordinal)) +
        geom_col(position = "dodge")
